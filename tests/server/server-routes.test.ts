...
describe("server collection and preview routes", () => {
  ...
  test("renders preview HTML, CSS, and preview-processed image assets", async () => {
    ...
    expect(previewHtml).toContain("gridgen-preview-shell");
    expect(previewHtml).toContain("background:#08080a");
    ...
    expect(previewCss).toContain(".gridgen-grid");
    expect(previewCss).toContain(".gridgen-collection--poster");
    expect(previewCss).toContain("background: transparent;");
  ...
  test("reports preview asset failures without claiming success", async () => {
    ...
    const collectionWithoutImage = unwrapOk(createCollectionDraft({ title: "Music" }));
    const sectionId = collectionWithoutImage.sections[0]?.id;

    if (sectionId === undefined) {
      throw new Error("Expected starter section.");
    }

    const itemWithoutImage = unwrapOk(
      updateCollection(collectionWithoutImage, {
        sectionId,
        title: "Album A",
